...
## TASK-P4-05 local implementation evidence

新增7个unit、3个fixed-seed property、10个independent mutation tests和1个CI machine-contract test，共21项Task-specific断言；加上machine command可复验versioned policy、COMPLETED/RUNNING、两类HARD、SOFT、half-open boundary、ADDED、immutability、exact replay、Snapshot→Problem exact-lock equivalence及4类fail-closed输入。`p4-freeze-window-report.v1`为7/7且`issues=[]`；完整Backend 675项及implementation artifact `9648715231`已exact复验。Registry仍为61个稳定Test ID，不分配新ID，只把TEST-FREEZE-WINDOW-001及既有TEST-RUNNING/INF-LOCK/VALIDATOR-MUTATION/PROPERTY/SIM-ISOLATION的P4-05 edge标为`PROVIDER_VERIFIED`。

## TASK-P4-04 provider-verified implementation evidence

本Task新增4个unit、2个property与4个migration-backed integration tests，覆盖11种event fact mapping、Snapshot replay/immutability、ledger/audit exact replay、checkpoint lost-response replay、gap/terminal/reference/plane rejection、Urgent Standard Import和末端audit故障原子rollback；Task-specific 10项连同application boundary与CI contract为focused `12 passed`，完整Backend `654 passed`。Frontend 67 Vitest、主E2E与两轮Gate Chromium各12/12、全部历史machine、P2/P3双Gate、SCA/license、Compose和双build均PASS；machine report为8/8且`issues=[]`。Implementation `47f55b41e370aa9d24fd9c987cff4663672c3ee8` / artifact `9644190441`已exact复验上述结果，因此TEST-EXECUTION-EVENT-CONTRACT-001的P4-04 runtime edge及TEST-EXECUTION-FACT-PROJECTION-001、TEST-SNAPSHOT-REPLAY-001、TEST-IDEMPOTENCY、TEST-PROPERTY、TEST-SIM-ISOLATION的P4-04 edge均为`PROVIDER_VERIFIED`；P4 Gate/Audit与Production evidence仍PLANNED。


## TASK-P4-03 provider-verified persistence evidence

新增4项unit与6项integration用例覆盖internal record determinism、既有terminal-state registry、无ReplanRequest业务状态、ExecutionEvent exact replay/position conflict、checkpoint/request原子CAS、attempt/result lineage、audit replay、caller-owned rollback、DB append-only guards、Production default-deny及P3/P4 plane隔离。连同migration与CI contract回归的focused组合为`51 passed`，完整Backend为`643 passed`；Frontend为67 Vitest、主E2E与两轮Gate Chromium各12/12，全部历史machine、P2/P3双Gate、SCA/license、Compose和双build均通过。`p4-replan-persistence-report.v1`为9/9且`issues=[]`。Implementation `60f8e8900ecab60f0d64311912ae27f09a4d002f` / artifact `9639720666`已把该persistence slice升级为`PROVIDER_VERIFIED`；closure自身仍须exact provider复验。

本Task没有新增Test ID或修改既有Test ID语义；registry仍为61项、`registry_version=1.0.0`。在P4-03 closure的历史时点P4-04事实投影仍为`PLANNED`；当前其provider状态见上方P4-04 section，P4-05 freeze、P4-06 ChangeReport计算、P4-07 Solver/Validator、P4-08应用及Production数据库/容量证据继续`PLANNED`。

## TASK-P4-02 machine-contract evidence

`test_p4_machine_contracts.py`现覆盖9/9 strict/offline/non-interchangeable Schema/sample、canonical bundle lineage、ExecutionEvent identity/authority、ReplanRequest无状态与half-open freeze、objective/status、complete ChangeReport、Simulator/export/state boundary及历史/依赖/migration freeze。`p4-machine-contract-report.v1`另执行35个Schema rejection与7个semantic rejection；implementation `539cdbbdcdd406daba25b8d6b8caaa5133691e76` / artifact `9636892191`已把该机器合同证据升级为`PROVIDER_VERIFIED`，但不得解释为event ingress、Solver、Simulator或Production行为PASS。
...
| TEST-CONTRACT-001 | Schema meta/positive/negative、版本、UTC/duration/reference、isolation 与 round-trip | P0-P4 | Existing suite + [`test_p4_machine_contracts.py`](../../backend/tests/contract/test_p4_machine_contracts.py)；historical bytes与additive `2.8.0` carrier slice PROVIDER_VERIFIED |
...
| TEST-SNAPSHOT-REPLAY-001 | Snapshot canonical bytes/hash/ID、immutability与 repository replay | P1-P4 | TASK-P1-08 suites formed/provider `32310098594` PASS；P1 pipeline full-byte replay formed；TASK-P4-04 event-derived Snapshot predecessor/bytes/hash/lost-response replay `PROVIDER_VERIFIED` / artifact `9644190441` |
...
| TEST-STATE-TRANSITION-001 | 三套 state enum、42 个 allowed pair、terminal/negative transitions | P0-P4 | frozen rules/carrier + P3 repository/lifecycle/command/decision provider-verified；P4-03复验既有terminal set且checkpoint CAS不是业务pair，PROVIDER_VERIFIED |
...
| TEST-INF-LOCK | Lock 导致的不可行性 | P0-P4 | P0/formal/P2-07 infeasible slices + [P2 Hard Lock positive and exact C-008 mutation](../../backend/tests/validation/test_p2_solver_mutations.py) formed；TASK-P4-05 fact/HARD/freeze conflict precheck `PROVIDER_VERIFIED` / artifact `9648715231` |
...
| TEST-RUNNING | 运行中事实保护 | P0-P4 | P0/P1/formal/P2-07 slices + [`P2-RUNNING@1.0.0` Solver replay](../../backend/tests/simulation/test_p2_scenario_matrix.py) and C-007 mutation formed；TASK-P4-05 COMPLETED/RUNNING exact fact evidence `PROVIDER_VERIFIED` / artifact `9648715231` |
...
| TEST-VALIDATOR-MUTATION | 独立 Validator 拒绝人工错误计划 | P0-P4 | P0/formal suites + [11 formula-free mutations of Solver-produced candidates](../../backend/tests/validation/test_p2_solver_mutations.py) formed；TASK-P4-05 independent freeze mutations `PROVIDER_VERIFIED` / artifact `9648715231`；performance integration PLANNED |
| TEST-REPLAN | Replan 事实、锁与变化报告 | P4 | Freeze/effective-lock preparation `PROVIDER_VERIFIED` / TASK-P4-05 / artifact `9648715231`；OBJ-002/ChangeReport/Solver/application仍PLANNED |
| TEST-EXECUTION-EVENT-CONTRACT-001 | ExecutionEvent版本、来源、时间、entity refs、幂等与拒绝合同 | P4 | MACHINE_CONTRACT_PROVIDER_VERIFIED / TASK-P4-02；ingress/projection runtime `PROVIDER_VERIFIED` / TASK-P4-04 / artifact `9644190441` |
| TEST-REPLAN-REQUEST-CONTRACT-001 | immutable base/new Snapshot/freeze/reason/request/result carrier合同 | P4 | MACHINE_CONTRACT_PROVIDER_VERIFIED / TASK-P4-02；P4-03 persistence PROVIDER_VERIFIED；application P4-08 PLANNED |
| TEST-P4-PERSISTENCE-001 | Event ledger、ReplanRequest/result、状态事务、CAS、audit与migration replay | P4 | PROVIDER_VERIFIED / TASK-P4-03；10 new focused、51 combined、machine 9/9 |
| TEST-EXECUTION-FACT-PROJECTION-001 | ExecutionEvent→权威事实→新immutable Snapshot确定性投影 | P4 | 11 event types + negative/atomic/standard urgent `PROVIDER_VERIFIED` / TASK-P4-04 / artifact `9644190441` |
| TEST-FREEZE-WINDOW-001 | Freeze边界、COMPLETED/RUNNING/HARD/effective lock保护与Production no-default | P4 | unit/property/mutation/machine `PROVIDER_VERIFIED` / TASK-P4-05 / artifact `9648715231` |
| TEST-STABILITY-OBJECTIVE-001 | OBJ-002 resource/start/count movement整数计算与lexicographic stage | P4 | PLANNED / TASK-P4-06/07 |
| TEST-CHANGE-REPORT-001 | before/after KPI、变化、facts/locks、reason与lineage完整性 | P4 | MACHINE_CONTRACT_PROVIDER_VERIFIED / TASK-P4-02；calculation/application/export P4-06/08/11 PLANNED |
...
| TEST-IDEMPOTENCY | Import/Planning/Export/Publish/Event 幂等 | P0-P4 | generic/storage/command/decision/publication/export worker provider-verified；P4-03 event/request/attempt/result/audit exact replay/conflict PROVIDER_VERIFIED；P4-04 ingress/checkpoint/lost-response byte replay PROVIDER_VERIFIED / artifact `9644190441`；external side effects PLANNED |
...
| TEST-SIM-ISOLATION | Synthetic/Production 隔离 | P0-P4 | existing plane/lifecycle/decision/publication/export provider-verified；P4-03 repository Production default-deny与P3/P4 table isolation PROVIDER_VERIFIED；P4-04 event/urgent/projector Simulation-only PROVIDER_VERIFIED / artifact `9644190441`；P4-05 policy/base/Snapshot cross-plane and Production no-default `PROVIDER_VERIFIED` / artifact `9648715231`；separate Production DB/auth/publish target PLANNED |
...
| TEST-PROPERTY | 合法 Problem/投影的通用不变量 | P2-P4 | Problem v2/formal/Solver/reference/XS-S-M properties formed；TASK-P4-04 fixed-seed event replay/order/plane/authority properties PROVIDER_VERIFIED / artifact `9644190441`；TASK-P4-05 boundary/grid/exact replay properties `PROVIDER_VERIFIED` / artifact `9648715231` |
...
| TEST-AUDIT-TRAIL-001 | append-only actor/reason/correlation/before-after/version audit完整性 | P3-P4 | TASK-P3-03～10 business/denial audit + P3-13 UI/audit/download lineage provider-verified；P4-03 versioned append-only storage/rollback PROVIDER_VERIFIED；retention/SIEM PLANNED |
