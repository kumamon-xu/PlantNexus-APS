...
P3-09实现既有ExportJob aggregate：immutable source/target/idempotency identity，mutable state/attempt/lease/heartbeat/artifact/error/timestamps由repository CAS守护。Pure domain负责authorization、identity、carrier与audit构造；application负责事务；exporter负责确定性package；job composition只做claim→materialize→complete/fail。没有Schedule mutation、Publish调用、external aggregate或P4 model。
