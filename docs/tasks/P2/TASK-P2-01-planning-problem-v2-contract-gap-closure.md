...
Files allowed to change: `schemas/json/planning-problem.v2.schema.json`、`schemas/samples/planning-problem.v2.synthetic.json`、`schemas/data_dictionary.yaml`、`backend/app/planning/problem/__init__.py`、`backend/app/planning/problem/contracts.py`、`backend/app/planning/problem/builder.py`、`backend/app/planning/problem/hashing.py`、`backend/app/domain/validation.py`、`backend/tests/contract/test_schema_contracts.py`、`backend/tests/unit/test_planning_problem_builder.py`、`backend/tests/property/test_planning_problem_properties.py`、`backend/tests/golden/test_p1_problem_replay.py`及`Documents to update`中的明确文档；新增路径须在进入in_progress前逐字写入本卡。

Files forbidden to change: `backend/app/planning/backends/**`、`backend/app/planning/strategies/**`、fixture历史bytes、P1 schema bytes、OR-Tools dependency、Validator evaluator、Export/Benchmark/P3代码。
...
Acceptance commands: `uv run pytest -q backend/tests/contract/test_schema_contracts.py backend/tests/unit/test_planning_problem_builder.py backend/tests/property/test_planning_problem_properties.py backend/tests/golden/test_p1_problem_replay.py`；`uv run ruff check .`；`uv run pyright backend/app backend/tests`；`uv run python scripts/check_docs.py`；`uv run python scripts/check_docs.py --task docs/tasks/P2/TASK-P2-01-planning-problem-v2-contract-gap-closure.md --check-diff --report build/traceability/TASK-P2-01-report.json`；`git diff --check`。
