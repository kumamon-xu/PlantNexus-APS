...
| TEST-IDEMPOTENCY | Import/Planning/Export/Publish/Event 幂等 | P0-P4 | generic/storage/command/decision/publication/export worker provider-verified；P4-03 event/request/attempt/result/audit exact replay/conflict PROVIDER_VERIFIED；external side effects PLANNED |
...
| TEST-SIM-ISOLATION | Synthetic/Production 隔离 | P0-P4 | existing plane/lifecycle/decision/publication/export provider-verified；P4-03 repository Production default-deny与P3/P4 table isolation PROVIDER_VERIFIED；separate Production DB/auth/publish target PLANNED |
