...
## TASK-P4-04 local implementation evidence（provider pending）

本Task新增4个unit、2个property与4个migration-backed integration tests，覆盖11种event fact mapping、Snapshot replay/immutability、ledger/audit exact replay、checkpoint lost-response replay、gap/terminal/reference/plane rejection、Urgent Standard Import和末端audit故障原子rollback；Task-specific 10项连同application boundary与CI contract为focused `12 passed`，完整Backend `654 passed`。Frontend 67 Vitest、主E2E与两轮Gate Chromium各12/12、全部历史machine、P2/P3双Gate、SCA/license、Compose和双build均PASS；machine report为8/8且`issues=[]`。TEST-EXECUTION-FACT-PROJECTION-001、TEST-SNAPSHOT-REPLAY-001、TEST-IDEMPOTENCY、TEST-PROPERTY、TEST-SIM-ISOLATION的P4-04 edge现为`LOCAL_PASS_PROVIDER_PENDING`，不得提前写为provider-verified；P4 Gate/Audit与Production evidence仍PLANNED。
...
本Task没有新增Test ID或修改既有Test ID语义；registry仍为61项、`registry_version=1.0.0`。P4-04事实投影、P4-05 freeze、P4-06 ChangeReport计算、P4-07 Solver/Validator、P4-08应用及Production数据库/容量证据继续`PLANNED`。
...
| TEST-SNAPSHOT-REPLAY-001 | Snapshot canonical bytes/hash/ID、immutability与 repository replay | P1-P4 | TASK-P1-08 suites formed/provider `32310098594` PASS；P1 pipeline full-byte replay formed；TASK-P4-04 event-derived Snapshot predecessor/bytes/hash/lost-response replay `LOCAL_PASS_PROVIDER_PENDING` |
...
| TEST-EXECUTION-EVENT-CONTRACT-001 | ExecutionEvent版本、来源、时间、entity refs、幂等与拒绝合同 | P4 | MACHINE_CONTRACT_PROVIDER_VERIFIED / TASK-P4-02；ingress/projection P4-04 PLANNED |
...
| TEST-EXECUTION-FACT-PROJECTION-001 | ExecutionEvent→权威事实→新immutable Snapshot确定性投影 | P4 | 11 event types + negative/atomic/standard urgent `LOCAL_PASS_PROVIDER_PENDING` / TASK-P4-04 |
...
| TEST-IDEMPOTENCY | Import/Planning/Export/Publish/Event 幂等 | P0-P4 | generic/storage/command/decision/publication/export worker provider-verified；P4-03 event/request/attempt/result/audit exact replay/conflict PROVIDER_VERIFIED；P4-04 ingress/checkpoint/lost-response byte replay LOCAL_PASS_PROVIDER_PENDING；external side effects PLANNED |
...
| TEST-PROPERTY | 合法 Problem/投影的通用不变量 | P2-P4 | Problem v2/formal/Solver/reference/XS-S-M properties formed；TASK-P4-04 fixed-seed event replay/order/plane/authority properties LOCAL_PASS_PROVIDER_PENDING |
