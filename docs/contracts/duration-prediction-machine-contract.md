...
## TASK-P6-08 deterministic aggregate monitoring and default-disable evidence

`duration-monitoring-policy.v1` is a strict, content-addressed Simulation/Test policy. The approved `SIM-P6-DURATION-MONITORING-001@1.0.0` identity binds the exact P6-06 model and feature versions, `SIM-ASSUMPTION-026`, four reference buckets with two expected observations each, an eight-observation window, fallback rate at most `1/4`, feature total variation at most `1/4`, quality pass ratio at least `3/4`, late count zero, and run-scoped retention of at most one window with no persistence. Unknown or duplicate fields, non-finite values, invalid or recomputed-unapproved identity, mixed lineage, and invalid thresholds fail closed.

The internal `duration-monitoring-window.v1` admits only explicit UTC bounds, exact policy/model/feature lineage and integer aggregate counts. Raw features, labels, FeatureRecords, operation/resource/source/row/user identifiers, credentials, free text and private-looking keys are forbidden recursively before metric evaluation. Counts must be nonnegative and closed against the observation total; bucket keys are exact. Invalid input is never reflected into output and produces a sanitized default-disable report.

Metrics use exact rational arithmetic and inclusive boundaries. Version, late, fallback-rate, feature-distribution, quality, privacy, tamper, insufficient-telemetry and window-count failures emit only registered monitoring reasons and `recommendation=DEFAULT_DISABLE`; the corresponding runtime requirement is the existing `fallback_reason=DRIFT_GATE_DISABLED` with exact authoritative standard duration. A healthy or exact-boundary window yields `NO_DISABLE_RECOMMENDATION`, never an enable or promotion decision.

`p6-duration-monitoring-check-report.v1` binds Task, exact code commit, Diff base, policy/window fingerprints, aggregate counts, exact ratios, checks, reasons and recommendation. It is an internal machine evidence envelope, not an additive JSON Schema or API carrier. The checker performs 12 deterministic checks covering healthy/boundary, fallback/feature/quality/version drift, late/window mismatch, privacy, tamper, replay, retention/isolation, no automatic actions and bounded development overhead. Reports contain no raw records or identifiers.

The monitor is pure and advisory: it has no network, telemetry store, model registry, cache, database, alert, state mutation, retraining, promotion, rollback or Planning output authority. Removing it leaves P6-06 standard-duration fallback and P6-07 default-off behavior intact. Production monitoring, retention, thresholds, ownership and SLO remain unformed.
...
The 50 ms per-call post-check, 16 KiB FeatureRecord, four features, one source, 32 KiB output, 256 measured/16 warmup, P95 20 ms and 16 MiB peak limits are fixed Simulation/Test safety evidence, not a Production SLA. P6-06 itself formed no PlanningProblem, Solver, Validator, API/UI, state, promotion or monitoring consumer; P6-07 later added only the separately authorized adapter above, and P6-08 later added only the advisory aggregate monitor above.
...
The separately authorized P6-03/P6-04 packages implement one synthetic contract-correctness dataset and one deterministic baseline model/training replay; those packages do not themselves implement an evaluation Gate or confidence policy. P6-05 adds the separate development-only offline evaluation/fallback Gate, P6-06 the explicit local Simulation/Test provider, P6-07 only the default-off Planning adapter, and P6-08 only the aggregate advisory monitor described above. None can alter routing, resource compatibility, hard constraints, PlanningRun/ScheduleVersion/ExportJob state or weights, and no API, migration, business state or Production authority exists. `AI_DURATION_PREDICTION` therefore remains DEFERRED with `AGGREGATE_MONITOR_V1 / DEFAULT_OFF_SIMULATION_ONLY`.

The v1 contract bytes consumed by P6-03/P6-04/P6-05/P6-06/P6-07/P6-08 are immutable; semantic change requires a new document/set or artifact/policy version. Runtime/ingress/monitor rollback disables or removes the local provider/adapter/monitor and keeps the frozen standard Problem and authoritative standard duration while preserving dataset/model/evaluation/prediction/decision/monitor lineage evidence. TASK-P6-09 and later require separate authorization and a new Diff base.
