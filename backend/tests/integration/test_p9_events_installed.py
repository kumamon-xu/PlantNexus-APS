"""Install the current wheel and replay event/projection HTTP contracts outside the checkout.
...
def test_events_http_contracts_from_installed_wheel(
    ...
    installed_test = tmp_path / "test_p9_events.py"
    installed_test.write_bytes(
        (root / "backend/tests/integration/test_p9_events.py").read_bytes()
    )
    ...
    report = root / "build/validation/P9-06"
