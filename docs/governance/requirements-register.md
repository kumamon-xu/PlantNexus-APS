...
TASK-P3-10 local review：REQ-001获得Data Health/Import/Planning read的strict HTTP carrier；REQ-004/005获得ScheduleVersion/validate与fresh application delegation边界；REQ-006获得workspace/comparison/export的17个HTTP operation；REQ-007获得server-derived authorization、command/decision/publication/export的strict precondition和error mapping；REQ-009获得correlation/OpenAPI/machine lineage。这些只是local transport slice，exact provider待提交后复验；Frontend、external/P4/Production未形成，所有root继续`ALLOCATED`、registry version不变。
