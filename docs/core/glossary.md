...
| Schema Set | 同一发布批次的机器合同集合；当前为 `2.0.0`，保留 `1.0.0/1.1.0/1.2.0` artifacts，且不替代各 document/asset/registry version ID |
...
当前Schema Set为`2.0.0`；`1.0.0`是TASK-P0-03数据合同发布，`1.1.0` additive增加rule/state/error/capability contracts，`1.2.0` additive增加FactoryProfile/ScenarioSpec/ScenarioManifest v1，`2.0.0` breaking set release新增canonical-records.v1、Import v2与Snapshot v2。全部历史artifact保留，document version仍需显式选择。
