...
| Task | 目标 | 依赖 | 状态 |
|---|---|---|---|
| [TASK-P2-00](P2/TASK-P2-00-phase-transition-and-task-planning-governance.md) | Phase transition、Task plan与batch CI治理 | P1-12 | `in_progress` |
...
TASK-P2-00只完成本次治理工作；P2-01～14未获本次实现授权且均为`planned`。P2-00 closure后建议首先另行授权P2-01。每个后续Task完成本地验收、提交并直接push当前`main`后，必须核验exact required `validate`和artifact；失败run保留且阻断closure。
