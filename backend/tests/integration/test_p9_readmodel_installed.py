"""Install the current wheel and replay read/export HTTP contracts outside the checkout.
...
def test_readmodel_http_contracts_from_installed_wheel(
    ...
    installed_test = tmp_path / "test_p9_readmodel.py"
    installed_test.write_bytes(
        (root / "backend/tests/integration/test_p9_readmodel.py").read_bytes()
    )
    ...
    report = root / "build/validation/P9-05"
