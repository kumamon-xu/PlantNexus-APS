...
TASK-P0-10 review：workflow 的当前 Task diff/report 与 uploaded artifact 引用已有界交接，GitHub baseline run `32227247262` 也使原失败点可从 provider 侧追溯。这只加强 REQ-009 的 repository/CI provenance slice，不实现真实 source/Snapshot/Problem/Solver/export manifest 或业务 audit。REQ-001～015 全部继续 `ALLOCATED`，registry format version 不变；P1 Task 未创建。
