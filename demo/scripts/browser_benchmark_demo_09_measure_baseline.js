async (page) => {
  ...
  const index = await page.evaluate(() =>
    Number(sessionStorage.getItem("demo09-baseline-sample-index") || "0"),
  );
  ...
  for (const fragment of ["/bootstrap", "/factory", "/versions/"]) {
    requireValue(
  ...
  await page.evaluate(
    (nextIndex) => sessionStorage.setItem("demo09-baseline-sample-index", String(nextIndex)),
    index + 1,
  );
  return {
    status: "PASS",
    sample: {
      sample_id: `baseline_published-${role}-${String(sequence).padStart(2, "0")}`,
