"""Install the current wheel and replay asynchronous replan HTTP contracts outside the checkout.
...
def test_consumers_from_installed_wheel(
    ...
    installed_test = tmp_path / "test_p9_consumers.py"
    installed_test.write_text(
        (root / "backend/tests/integration/test_p9_consumers.py")
    ...
    bootstrap.write_text(
        ...
        "import pytest\n"
        f"raise SystemExit(pytest.main({['--noconftest', '-c', str(configuration), str(installed_test), '-k', 'not browser', '-q', '--junitxml', str(evidence)]!r}))\n",
        encoding="utf-8",
    )
    ...
    report = root / "build/validation/P9-08"
