...
| TEST-STATE-TRANSITION-001 | 三套 state enum、42 个 allowed pair、terminal/negative transitions | P0-P3 | frozen rules/carrier + P3 repository/lifecycle/command/decision provider-verified；TASK-P3-08 APPROVED→PUBLISHED/PUBLISHED→SUPERSEDED local evidence formed、provider pending |
...
| TEST-IDEMPOTENCY | Import/Planning/Export/Publish/Event 幂等 | P0-P3 | generic/storage/command/decision slices provider-verified；TASK-P3-08 publish replay/conflict/current CAS/rollback local evidence formed、provider pending；export/worker/external side effects PLANNED |
...
| TEST-SIM-ISOLATION | Synthetic/Production 隔离 | P0-P3 | existing plane/lifecycle/decision slices provider-verified；TASK-P3-08 internal Simulation target与Production pre-lookup denial local evidence formed、provider pending；separate Production DB/API/auth/publish target PLANNED |
...
| TEST-SCHEDULE-VERSION-LIFECYCLE-001 | Validated Solution→DRAFT→READY_FOR_REVIEW与非法状态拒绝 | P3 | TASK-P3-04 DRAFT→READY、TASK-P3-07 READY→APPROVED/REJECTED均provider-verified；TASK-P3-08 publication lifecycle local evidence formed、provider pending |
...
| TEST-APPROVAL-AUTHORIZATION-001 | authority-neutral capability、default-deny、approve/reject guards | P3 | domain/application/contract/integration/security + 8/8 machine exact provider PASS / TASK-P3-07；HTTP/UI E2E PLANNED / P3-10、13 |
| TEST-PUBLISH-IDEMPOTENCY-001 | APPROVED-only publish、same-key replay、conflict与supersession | P3 | unit/contract/integration/security + 8/8 machine local PASS / TASK-P3-08；exact provider pending |
...
| TEST-AUDIT-TRAIL-001 | append-only actor/reason/correlation/before-after/version audit完整性 | P3 | storage/command/decision slices provider-verified；TASK-P3-08 success+DENIED publication audit、redaction、rollback/replay local evidence formed；export/API completeness PLANNED / P3-09～10 |
...
当前focused 16项、full repository 577项及`p3-publication-report.v1`均PASS；machine为8/8、3 success、2 supersession、1 replay、1 conflict、2 denial、4无业务state拒绝、1 rollback、1 concurrent winner、Solver调用0、`issues=[]`。Test ID总数保持48、registry version保持`1.0.0`；exact implementation provider尚未形成，ExportJob/package、HTTP/UI、external/P4/Production测试继续PLANNED。
