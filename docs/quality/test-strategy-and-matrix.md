...
| TEST-IDEMPOTENCY | Import/Planning/Export/Publish/Event 幂等 | P0-P3 | generic/storage/command/decision + TASK-P3-08 publish replay/conflict/current CAS/rollback provider-verified；export/worker/external side effects PLANNED |
...
| TEST-SIM-ISOLATION | Synthetic/Production 隔离 | P0-P3 | existing plane/lifecycle/decision + TASK-P3-08 internal Simulation target与Production pre-lookup denial provider-verified；separate Production DB/API/auth/publish target PLANNED |
...
| TEST-PUBLISH-IDEMPOTENCY-001 | APPROVED-only publish、same-key replay、conflict与supersession | P3 | unit/contract/integration/security + 8/8 machine exact provider PASS / TASK-P3-08 |
| TEST-EXPORT-JOB-001 | ExportJob transition、package integrity、atomic/idempotent export与失败恢复 | P3 | persistence slice provider-verified / TASK-P3-03；business/package slice implementation candidate FORMED / TASK-P3-09，provider pending |
| TEST-AUDIT-TRAIL-001 | append-only actor/reason/correlation/before-after/version audit完整性 | P3 | storage/command/decision + TASK-P3-08 success/DENIED publication audit、redaction、rollback/replay provider-verified；export/API completeness PLANNED / P3-09～10 |
