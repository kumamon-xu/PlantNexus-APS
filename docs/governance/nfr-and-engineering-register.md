...
ENG-ARCH形成pure publication domain→repository/transaction ports application→composition-root；ENG-ERR使用module-local stable reasons；ENG-VER保持2.6.0/state/migration/dependency零漂移；ENG-LOG获得PUBLICATION append-only event。Exact provider尚未形成；SQLite不证明distributed concurrency/SLA，全部NFR/ENG root继续`ALLOCATED`，OPEN-002/010、RISK-007/008/011～013与`registry_version=1.0.0`不变。
