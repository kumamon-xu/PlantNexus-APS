...
Version/mixed lineage、fallback spike、feature/quality drift、late/incomplete window、threshold mismatch、tamper或privacy violation都会产生deterministic aggregate report，稳定建议`DEFAULT_DISABLE`并给出`DRIFT_GATE_DISABLED`，要求caller继续使用同resource option的权威standard duration。Monitor不切换model、不自动retrain/promotion/rollback、不写Planning/state、不发送external alert；`OPEN-010/011/014/015`、Production monitoring/SLO与P7 Reality Calibration保持未形成。

运行`uv run python scripts/p6_duration_monitoring_check.py --root . --report build/validation/p6-duration-monitoring-report.json`可重放12项monitoring Gate。既有`p6_duration_runtime_check.py`在FULL链中不可跳过地生成独立monitor report，workflow本身不变；development overhead只记录观测、不设Production threshold。
...
Implementation `bf75f601b0a8138ab80304bee68e9bbe34b124fd` / exact FULL run `33474633364`已成功：classify/FULL/required jobs `99751230649`/`99751254113`/`99753251628`均PASS，DOCS_ONLY `99751254977`正确skipped，required `validate`来自GitHub Actions app `15368`。73个workflow steps、950 repository tests、P6-04 10/10、14 mutation rejection、2 atomic rejection、P6-02/03 upstream及冻结P4 replay全部通过。未过期profile/evidence artifacts `9787709131`/`9787940077`的Provider与下载ZIP digests逐字一致为`sha256:f4d2ff5ec857a0f178d8900ad1b086cb60ff644eed6a9ea14f6fb4352d4479f2`、`sha256:3547433403a048bd13e9ce14bf0630649eb57ec7d3db7a09171238dafe123615`；profile精确绑定FULL/18 paths/base→head/`issues=[]`，66个下载JSON全部可解析且无non-empty issue/gap，model payload无raw source、dataset rows或labels。本提交只作两份README的evidence-only closure；closure exact provider成功前TASK仍为`in_progress`。
...
Implementation `19c80dac7cf298b423d9e22add2268421520ef75` / exact FULL run `33468228427`已成功：classify/FULL/required jobs `99732427284`、`99732449853`、`99734830107`全部PASS，DOCS_ONLY `99732451067`正确skipped，required `validate`来自GitHub Actions app `15368`。72个workflow steps、928 repository tests、P6-03 10/10、P6-02 10/10及冻结P4-13 replay全部通过。未过期profile/evidence artifacts `9785603625`/`9785860246`的Provider与下载ZIP digests一致为`sha256:80620dc08b8162027112c66ee49990687ad7919ced6bf1ecbf2757faab932fe2`、`sha256:c85f8fc92abbf58cf3a3533730d13f31ce33c61bc193a163af8595da597f5fc8`；profile精确为20 paths/FULL/`issues=[]`，machine复现10→8+2、4/2/2、12 rejection、source/manifest/bundle/report fingerprints及`issues=[]`。62个下载JSON全部可解析且无non-empty issue/gap；evidence ZIP无raw source/rows。本提交只作两份README的evidence-only closure；closure exact provider成功前TASK仍为`in_progress`。
...
Planning implementation `5a58356d8df45c9156223d2b4ca935cc3e5f2f7a`的run/classify/docs/required=`33456013298`/`99696008255`/`99696033155`/`99696075050`全部成功，required由GitHub Actions app `15368`提供，FULL `99696035089`按`DOCS_ONLY`正确跳过。未过期profile/public-doc artifacts `9781446596`,`9781450874`的Provider/下载ZIP digests一致为`sha256:eb49b95cf654d9296ec956966b96b09fd075b346392c39122320ad31391794f1`、`sha256:c46d694aeb5a2835d219427a3a99f28301e71438ed36c9c7c882b8e800ad9022`；报告精确绑定原P5 closure、3份公开Markdown、4/4+1/1 checks和`issues=[]`。本提交只作两份README的evidence-only closure；closure exact provider完成前TASK-P6-00仍`in_progress`，P6-01不自动启动。
...
本地结论为`READY`，但首个implementation candidate `5d60dc90ddf22e75ec1783c0a7b92c016d452136` / run `33450894193`在冻结P4-13 replay中正确fail closed：其隔离worktree虽已移除P5 Gate/Audit文件，却仍使用当前70-step CI contract，冻结P4证据要求的68-step治理断言因此缺失。失败run与artifacts `9779882690`,`9779713051`保留；direct corrective只在该临时worktree恢复P5-21 closure `d0a83c58cb4a2d4afa76e8c8cff08441574e2e30`的CI contract，再运行未修改P4-13 evidence，不改变主工作树测试断言、P4 owner或Audit结论。

Corrective implementation `83c51d765045da030b8cff871191be37bb9e899a`的run/classify/FULL/required=`33451784906`/`99683098887`/`99683143439`/`99685627036`全部成功，required `validate`由GitHub Actions app `15368`提供。Machine/profile artifacts `9780286365`,`9780011491`未过期，Provider/下载ZIP digests一致为`sha256:d3353e06754f1585baf628a9537686466f9460130d2a6aa3a4d92543a4cb63b6`、`sha256:e5b205cb673c3d53f8152b2ad6fb2c21c4f17ca9009a736bb9c4e2546571139e`；60 JSON全部解析、216个`code_commit`字段exact、0 non-empty issue/gap/scope。Exact Exit为15 paths、4 Impact Rules、15/15 READY、`issues=[]`、`blocking_gaps=[]`。本提交只作两份README的evidence-only closure；closure自身仍须post-push exact provider，完成前TASK保持`in_progress`。P5 Milestone继续`active`；READY不自动执行P5→P6，也不形成Production/UAT、真实approval authority、external publish/integration、deployment或capacity/SLA。
...
CI在既有68-step FULL路由中的P4 Gate step后追加执行P5报告器，保持P4冻结evidence和步骤计数不变。失败candidate `e0dee8544a27adcae7ca98fabe2665452bf38d4d`/run `33402484533`与`d00386f42fbd366afa94dae4cc93096c0242ce0e`/run `33403931397`均按fail-closed保留，分别暴露fixture commit identity和历史P4-13累计scope误归属。Corrective implementation `c8ffd042738ffe79c350262aa7195daa9a7bf083`的FULL run/classify/full/required jobs为`33406166742`/`99534176989`/`99534217662`/`99538066470`，required `validate`由GitHub Actions app `15368`成功提供。Machine/profile artifacts `9763658843`/`9763213551`的下载ZIP与Provider digest逐字一致：`sha256:ed29d5aa2891d6a442a91360391219e6c265f2a0fe6d0b5e4210bd4c5ae316ed`、`sha256:1375f3fbeb739b53dee819da977e21f875c8bdc0ca910da86f43b84cd3cd098c`。Exact P5报告为12/12、4 Impact Rules、`issues=[]`、`blocking_gaps=[]`；evidence-only closure exact provider完成前TASK仍保持`in_progress`。即使本Gate PASS，也不是P5 Exit Audit，不自动启动TASK-P5-22，不形成P6+、Production/UAT、真实approval authority、external publish/integration、deployment或capacity/SLA。
