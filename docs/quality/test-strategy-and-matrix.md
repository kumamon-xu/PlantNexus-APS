...
## TASK-P4-03 local persistence evidence

新增4项unit与6项integration用例覆盖internal record determinism、既有terminal-state registry、无ReplanRequest业务状态、ExecutionEvent exact replay/position conflict、checkpoint/request原子CAS、attempt/result lineage、audit replay、caller-owned rollback、DB append-only guards、Production default-deny及P3/P4 plane隔离。连同migration与CI contract回归的focused组合为`51 passed`，完整Backend为`643 passed`；Frontend为67 Vitest、主E2E与两轮Gate Chromium各12/12，全部历史machine、P2/P3双Gate、SCA/license、Compose和双build均通过。`p4-replan-persistence-report.v1`为9/9且`issues=[]`。这些状态为`LOCAL_PASS_PROVIDER_PENDING`，只有implementation与closure的exact artifact复核后才能升级为provider verified。
...
| TEST-STATE-TRANSITION-001 | 三套 state enum、42 个 allowed pair、terminal/negative transitions | P0-P4 | frozen rules/carrier + P3 repository/lifecycle/command/decision provider-verified；P4-03复验既有terminal set且checkpoint CAS不是业务pair，LOCAL_PASS_PROVIDER_PENDING |
...
| TEST-REPLAN-REQUEST-CONTRACT-001 | immutable base/new Snapshot/freeze/reason/request/result carrier合同 | P4 | MACHINE_CONTRACT_PROVIDER_VERIFIED / TASK-P4-02；P4-03 persistence LOCAL_PASS_PROVIDER_PENDING；application P4-08 PLANNED |
| TEST-P4-PERSISTENCE-001 | Event ledger、ReplanRequest/result、状态事务、CAS、audit与migration replay | P4 | LOCAL_PASS_PROVIDER_PENDING / TASK-P4-03；10 new focused、51 combined、machine 9/9 |
...
| TEST-IDEMPOTENCY | Import/Planning/Export/Publish/Event 幂等 | P0-P4 | generic/storage/command/decision/publication/export worker provider-verified；P4-03 event/request/attempt/result/audit exact replay/conflict LOCAL_PASS_PROVIDER_PENDING；external side effects PLANNED |
...
| TEST-SIM-ISOLATION | Synthetic/Production 隔离 | P0-P4 | existing plane/lifecycle/decision/publication/export provider-verified；P4-03 repository Production default-deny与P3/P4 table isolation LOCAL_PASS_PROVIDER_PENDING；separate Production DB/auth/publish target PLANNED |
...
| TEST-AUDIT-TRAIL-001 | append-only actor/reason/correlation/before-after/version audit完整性 | P3-P4 | TASK-P3-03～10 business/denial audit + P3-13 UI/audit/download lineage provider-verified；P4-03 versioned append-only storage/rollback LOCAL_PASS_PROVIDER_PENDING；retention/SIEM PLANNED |
