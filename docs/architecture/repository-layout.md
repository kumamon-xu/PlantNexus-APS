...
TASK-P0-01 已建立可构建的顶层仓库边界；以下树继续定义 P0 的目标责任结构。除 `backend/app/__init__.py` 的版本占位和 `scripts/check_docs.py` 的仓库治理检查外，本 Task 只提交顶层 `.gitkeep`，不提前创建深层模块实现。
...
| `backend/tests/` | 测试类型目录占位 | 测试由交付对应行为的 Task 增加 |
| `frontend/`、`schemas/`、`fixtures/`、`benchmarks/`、`infra/` | 目录占位 | 不表示 Frontend、Schema、Fixture、Benchmark 或基础设施已形成 |
| `scripts/check_docs.py` | P0-01 文档结构检查 | 完整 registry/traceability/diff 检查属于 TASK-P0-02 |
