...
P8-26在2026-09-14的fresh扫描中检测到同一CVE两条记录从UNKNOWN更新为MEDIUM，原v1严格门因此阻断，失败Provider保留。重新读取Debian公告并对相同镜像执行完整nscd缺失检查后，新增`nscd-advisory.v2.json`，显式绑定旧评估SHA、当日公告SHA、旧/新severity和评估日期。当前构建消费v2；v1仍只接受旧UNKNOWN事实，v2只接受MEDIUM且其余CVE/包/版本/状态/无fix条件全部相同。其他severity、缺失/改变的provenance、组件存在或有FixedVersion仍拒绝；不把一次重新评估变成通用severity例外。原260条未解决风险、镜像内容与Runtime/Kit输入保持不变。
