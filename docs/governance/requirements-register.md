...
TASK-P3-09 implementation candidate：REQ-006获得PUBLISHED-only durable ExportJob、standard JSON/CSV/XLSX、manifest-last、retry/cancel/recovery和Publish分离；REQ-007获得server authorization、既有state pair/lease CAS及ScheduleVersion不变边界；REQ-009获得P2→Version→Publication→Job attempt→Audit→manifest/file exact lineage。16 focused与8/8 machine已形成但provider pending；API/UI、external/P4/Production仍未形成，全部root继续`ALLOCATED`且registry version不变。
