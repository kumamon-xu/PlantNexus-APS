...
def package_inputs() -> tuple[InternalExportPackage, dict[str, object], dict[str, object], dict[str, object]]:
    ...
    schedule = json.loads((ROOT / "schemas/samples/schedule-version.v1.synthetic.json").read_text(encoding="utf-8"))
    ...
    schedule["decision"] = {
        "decision": "APPROVED", "actor_ref": "actor:sim-approver-001", "capability": "approve",
        "reason": "Synthetic export fixture approval.", "decided_at_utc": "2026-08-25T00:00:00Z", "audit_event_id": "audit-approve-export-fixture",
    }
    schedule["publication"] = {
        "publication_id": "publication-export-fixture", "target": "SIMULATION_INTERNAL",
        "published_at_utc": "2026-08-25T00:01:00Z", "audit_event_id": "audit-publish-export-fixture",
    }
    schedule["allowed_actions"] = ["view", "export"]
    require_workspace_document(schedule)

    publication = json.loads((ROOT / "schemas/samples/publication-result.v1.synthetic.json").read_text(encoding="utf-8"))
    reference = {
    ...
    context = ExportJobContext(
        actor_ref="actor:export-worker", authenticated=True,
        resolved_capabilities=frozenset({"export"}),
        schedule_version_scope=frozenset({str(schedule["schedule_version_id"])}),
        export_job_scope=frozenset(), auth_policy_version="sim-policy.v1",
        production_binding=False, occurred_at_utc="2026-08-25T00:02:00Z",
    ...
    exporting = transition_export_job(
        created, target_state="EXPORTING", occurred_at_utc="2026-08-25T00:03:00Z",
        audit_event_id_value=audit_event_id(identity.export_job_id, "ATTEMPT", attempt),
        attempt=attempt, lease_reference=lease_reference_for(identity.export_job_id, attempt, "worker:test"),
    )
    return p2, schedule, publication, exporting


def _build(inputs: tuple[InternalExportPackage, dict[str, object], dict[str, object], dict[str, object]]):  # type: ignore[no-untyped-def]
    ...
    return build_standard_export_package(
        ...
        completion_audit_event_id=audit_event_id(str(job["export_job_id"]), "COMPLETED", attempt),
        correlation_id="correlation-export-fixture",
        generated_at_utc="2026-08-25T00:04:00Z",
    )


def test_standard_package_is_byte_deterministic_and_preserves_p2_payloads(package_inputs) -> None:  # type: ignore[no-untyped-def]
    ...
    for path in ("schedule_operations.csv", "order_summary.csv", "resource_load.csv", "kpi.json", "validation_report.json", "solver_report.json", "import_quality_report.json", "scenario_manifest.json"):
        assert first.files[path] == p2.files[path]
    assert first.manifest["file_count"] == 12
    assert first.manifest["deferred_artifacts"][1]["status"] == "DEFERRED_P4_DYNAMIC_REPLAN"
...
def test_xlsx_has_fixed_safe_sheets_and_no_active_content(package_inputs) -> None:  # type: ignore[no-untyped-def]
    ...
    assert workbook.sheetnames == ["Schedule Operations", "Order Summary", "Resource Load", "Metadata"]
    ...
    with ZipFile(io.BytesIO(value)) as archive:
        assert all("externalLinks/" not in name and "vbaProject.bin" not in name for name in archive.namelist())
...
def test_tamper_and_mixed_lineage_fail_closed(package_inputs) -> None:  # type: ignore[no-untyped-def]
    ...
    files = package.files
    files["kpi.json"] += b" "
    tampered = StandardExportPackage(package.package_id, package.manifest_fingerprint, package.storage_reference, tuple(sorted(files.items())))
    ...
    with pytest.raises(StandardExportError) as mixed:
        build_standard_export_package(
            p2_package=package_inputs[0], schedule_version=wrong_schedule,
            publication_result=publication, export_job=package_inputs[3],
            create_audit_event_id="audit-create", attempt_audit_event_id="audit-attempt",
            completion_audit_event_id="audit-complete", correlation_id="correlation",
            generated_at_utc="2026-08-25T00:04:00Z",
        )
    assert mixed.value.code is StandardExportErrorCode.MIXED_LINEAGE


def test_manifest_last_atomic_replay_conflict_and_cleanup(package_inputs, tmp_path: Path) -> None:  # type: ignore[no-untyped-def]
    package = _build(package_inputs)
    ...
    assert write_standard_export_package(package, destination, file_writer=recording_writer) == destination
    ...
    with pytest.raises(StandardExportError):
        write_standard_export_package(package, failed_destination, file_writer=failing_writer)
    assert not failed_destination.exists()
    assert not list(tmp_path.glob(".failed-attempt.tmp-*"))


def test_worker_composes_claim_package_storage_and_completion_without_publish(package_inputs, tmp_path: Path) -> None:  # type: ignore[no-untyped-def]
    ...
    class FakeService:
        ...
        def complete(self, _job_id: str, context: ExportJobContext, *, expected_lease_reference: str, artifact_manifest: Mapping[str, object]) -> ExportJobServiceResult:
            assert expected_lease_reference == exporting["lease_reference"]
            self.artifact = artifact_manifest
            completed = transition_export_job(
                exporting, target_state="EXPORTED", occurred_at_utc=context.occurred_at_utc,
                audit_event_id_value=audit_event_id(str(exporting["export_job_id"]), "COMPLETED", 1),
                artifact_manifest=artifact_manifest,
            )
            return ExportJobServiceResult(completed, 2, str(completed["latest_audit_event_id"]), False)
    ...
    fake = FakeService()
    context = ExportJobContext(
        actor_ref="actor:export-worker", authenticated=True, resolved_capabilities=frozenset({"export"}),
        schedule_version_scope=frozenset({str(schedule["schedule_version_id"])}),
        export_job_scope=frozenset({str(exporting["export_job_id"])}),
        auth_policy_version="sim-policy.v1", production_binding=False,
        occurred_at_utc="2026-08-25T00:04:00Z", code_commit="uncommitted",
    )
    worker = InternalExportJobWorker(service=cast(Any, fake), storage_root=tmp_path)
    result = worker.run(
        export_job_id=str(exporting["export_job_id"]), claim_context=context,
        terminal_context=context, owner_reference="worker:test", lease_expires_at_utc=datetime(2026, 8, 25, 0, 10, tzinfo=UTC),
        p2_package=package_inputs[0], schedule_version=schedule, publication_result=publication,
        correlation_id="correlation-export-fixture",
    )
    assert result.job.document["state"] == "EXPORTED"
    assert result.destination.is_dir() and (result.destination / "manifest.json").is_file()
    assert fake.artifact is not None and fake.artifact["export_manifest_version"] == "export-manifest.v2"
