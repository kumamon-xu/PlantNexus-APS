...
新增7个unit、3个fixed-seed property、10个independent mutation tests和1个CI machine-contract test，共21项Task-specific断言；加上machine command可复验versioned policy、COMPLETED/RUNNING、两类HARD、SOFT、half-open boundary、ADDED、immutability、exact replay、Snapshot→Problem exact-lock equivalence及4类fail-closed输入。`p4-freeze-window-report.v1`为7/7且`issues=[]`；exact full-suite、Task report和provider结果将在implementation验收与evidence-only closure回填。Registry仍为61个稳定Test ID，不分配新ID，只把TEST-FREEZE-WINDOW-001及既有TEST-RUNNING/INF-LOCK/VALIDATOR-MUTATION/PROPERTY/SIM-ISOLATION的P4-05 edge标为`LOCAL_PASS_PROVIDER_PENDING`。
...
| TEST-INF-LOCK | Lock 导致的不可行性 | P0-P4 | P0/formal/P2-07 infeasible slices + [P2 Hard Lock positive and exact C-008 mutation](../../backend/tests/validation/test_p2_solver_mutations.py) formed；TASK-P4-05 fact/HARD/freeze conflict precheck `LOCAL_PASS_PROVIDER_PENDING` |
...
| TEST-RUNNING | 运行中事实保护 | P0-P4 | P0/P1/formal/P2-07 slices + [`P2-RUNNING@1.0.0` Solver replay](../../backend/tests/simulation/test_p2_scenario_matrix.py) and C-007 mutation formed；TASK-P4-05 COMPLETED/RUNNING exact fact evidence `LOCAL_PASS_PROVIDER_PENDING` |
...
| TEST-VALIDATOR-MUTATION | 独立 Validator 拒绝人工错误计划 | P0-P4 | P0/formal suites + [11 formula-free mutations of Solver-produced candidates](../../backend/tests/validation/test_p2_solver_mutations.py) formed；TASK-P4-05 independent freeze mutations `LOCAL_PASS_PROVIDER_PENDING`；performance integration PLANNED |
| TEST-REPLAN | Replan 事实、锁与变化报告 | P4 | Freeze/effective-lock preparation `LOCAL_PASS_PROVIDER_PENDING` / TASK-P4-05；OBJ-002/ChangeReport/Solver/application仍PLANNED |
...
| TEST-FREEZE-WINDOW-001 | Freeze边界、COMPLETED/RUNNING/HARD/effective lock保护与Production no-default | P4 | unit/property/mutation/machine `LOCAL_PASS_PROVIDER_PENDING` / TASK-P4-05 |
...
| TEST-PROPERTY | 合法 Problem/投影的通用不变量 | P2-P4 | Problem v2/formal/Solver/reference/XS-S-M properties formed；TASK-P4-04 fixed-seed event replay/order/plane/authority properties PROVIDER_VERIFIED / artifact `9644190441`；TASK-P4-05 boundary/grid/exact replay properties `LOCAL_PASS_PROVIDER_PENDING` |
