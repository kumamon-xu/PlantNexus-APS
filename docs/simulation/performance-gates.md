...
## Corrective candidate replay

The same runner accepts an explicit `p9-audit-candidate.v1` identity for Runtime 0.2.1 / Kit 1.1.1. The delivery gate writes the exact source SHA and archive hashes only after reproducible assembly, clean installation, upgrade/restore and supply-chain checks succeed. The independent audit verifies that identity, archive manifests, nested Runtime bytes and installed module paths before running the unchanged 48 checks in a fresh environment. The corrective suite uses valid carriers and compares durable business rows before and after rejection. A corrective NOT_READY remains a failure for normal CI collection. Omitting the identity retains the original pinned P9-10 audit inputs. Historical NOT_READY evidence and budgets remain unchanged.
